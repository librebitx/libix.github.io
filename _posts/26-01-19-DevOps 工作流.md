...
<div class="mermaid">
sequenceDiagram
    participant A as 🎮 控制中心 (Zone A)
    participant B as 🏗️ 测试环境 (Zone B)
    participant C as 🏭 生产环境 (Zone C)
rect rgb(240, 240, 240)
Note right of A: 阶段 1: 持续集成与验证
A->>B: 下发部署指令 (Config + Image)
B->>B: 压力测试 & 安全扫描
B-->>A: 反馈验证结果
end

rect rgb(230, 245, 255)
Note right of A: 阶段 2: 准入与审批
A->>A: 触发人工审批流
Note over A: 管理员点击 [Approve]
end

rect rgb(255, 240, 240)
Note right of A: 阶段 3: 生产发布
A->>C: 执行蓝绿发布/灰度发布
C-->>A: 实时监控异常 (Anomaly Detection)
opt 发现异常
    A->>C: 触发自动回滚 (Rollback)
end
end
